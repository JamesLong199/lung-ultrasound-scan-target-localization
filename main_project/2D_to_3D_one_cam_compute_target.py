...
# Compute 3D target prediction using only one cam
# This script wasn't used in our project in the end.
...
def target12_3D(l_shoulder, r_shoulder, ratio_1=0.3, ratio_2=0.1):
    X1 = convert_depth_to_phys_coord_using_realsense(int(r_shoulder[0]), int(r_shoulder[1]), depth_data, depth_intr)  # in the cam frame
...
def target4_3D(r_shoulder, r_hip, ratio_1=0.35, ratio_2=0.1):
    ...
    target = post_process(target)
    print("post process target 1: ", target)
    target2 = post_process_2(target)
    print("post process target 2: ", target2)
    return target
...
def post_process_2(original_3d):
    idx = np.argmin(np.square(coordinates - original_3d.squeeze()).sum(axis=1))
...
with open(folder_path + POSE_MODEL + '/final_target.pickle', 'rb') as f:
    two_cam_target4 =  pickle.load(f)
print("two_cam_target4: \n", two_cam_target4)
