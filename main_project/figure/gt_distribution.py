# Plot the ground-truth xyz position distribution of 3 targets
...
for i, axis_name in enumerate(['X-axis', 'Y-axis', 'Z-axis']):
    ...
    if i == 1:
        ax[i].legend([bp_list[0]["boxes"][0], bp_list[1]["boxes"][0], bp_list[2]["boxes"][0]],
                   ['Cam1 Only', 'Two Cams', 'Cam2 Only'], loc='lower center', prop={'size': 22}, bbox_to_anchor=(0.5, -0.27), ncol=3)

    ax[i].set_xticks([(a + 1) * BIG_INTERVAL + SMALL_INTERVAL for a in range(3)], ['Target1', 'Target2', 'Target4'], fontsize=22)
    ax[i].tick_params(axis='y', labelsize=18)
    if i == 0:
        ax[i].set_ylabel('Ground-truth Position (mm)', fontsize=22)
    ax[i].set_title(axis_name, fontsize=27)

plt.savefig('gt_distribution_bigger_text.pdf', bbox_inches='tight')
plt.show()