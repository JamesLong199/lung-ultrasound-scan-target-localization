...
def heatmap(data, row_labels, col_labels, ax=None, add_xticks=False, add_yticks=False, title=None, xlabel=None,
    ...
    **kwargs
    ...
    # Plot the heatmap, fix the minmax range.
    im = ax.imshow(data, vmin=0, vmax=100, **kwargs)

    # Show all ticks and label them with the respective list entries.
    if add_xticks:
        ax.set_xticks(np.arange(data.shape[1]), labels=col_labels, fontsize=13)
    else:
        ax.set_xticks([])
    if add_yticks:
