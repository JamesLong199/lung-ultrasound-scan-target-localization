...
def localize_camera(fname, cam_matrix, dist_coeffs, tag_list):
    ...
    myCPD = TagDetector(cam_matrix, dist_coeffs, tag_families, "standard")
    img = cv2.imread(fname)
    _, det_list = myCPD.detect_tags(img, tag_size=tag_size, verbose=False)
...
def group_detection_result(img_dir, cam_matrix, dist_coeffs, tag_list, display_all=False):
    ...
    if display_all == False:
        ...
        result_dict = {'img_name': [], '# tags': [], 'pos': [], 'avg_pos': [], 'std_pos': [], 'angles': [],
                       'avg_angles': [], 'std_angles': []}
